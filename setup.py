...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_webvtt", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_webvtt": ["*.pyi", "py.typed"],
        "tree_sitter_webvtt.queries": ["*.scm"],
    },
    ext_package="tree_sitter_webvtt",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_webvtt/binding.c",
                "src/parser.c",
            ],
            define_macros=[
